...
SELECT first_name, last_name, salespeople.email FROM salespeople WHERE region = 'Northwest';
...
SELECT salespeople.email FROM salespeople WHERE region = 'Northwest' OR region = 'Southwest';
...
SELECT salespeople.email FROM salespeople WHERE region in ('Northwest', 'Southwest');
...
SELECT salespeople.email, first_name, last_name FROM salespeople WHERE region in ('Northwest', 'Southwest') AND last_name LIKE 'M%';
...
SELECT AVG(order_total) AS AVG_sum FROM orders;
...
SELECT MIN(order_total) AS MIN_total FROM orders;
...
SELECT melons.common_name, melons.melon_type, order_items.quantity, order_items.unit_price, order_items.total_price FROM order_items JOIN melons ON order_items.melon_id=melons.id WHERE order_id = 2725;
...
SELECT SUM(order_total) FROM orders GROUP BY salesperson_id HAVING SUM(order_total) = 1342366.11;